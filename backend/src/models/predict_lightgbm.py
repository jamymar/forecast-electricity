...
def is_bridge(dt):
    """Calcule si un jour fait le pont en France."""
...
def prepare_features(df):
    """Génère toutes les variables explicatives pour LightGBM, incluant la météo et le calendrier."""
    df = df.copy()
    ...
    # 4. Lags de consommation historiques
    ...
    # Conversion explicite des variables qualitatives en type 'category' pour LightGBM
    categorical_cols = ['hour', 'dayofweek', 'month', 'is_weekend', 'is_school_holiday', 'is_holiday', 'is_bridge_day']
...
def main():
    logging.info("--- Début du pipeline LightGBM Thermosensible ---")
    ...
    # Définition des colonnes de base à l'instant T
    base_feature_cols = [
    ...
    models = {}
    logging.info("Entraînement des 48 modèles LightGBM (stratégie directe)...")
    ...
    for h in range(1, 49):
        ...
        # Modèle LightGBM
        # num_leaves, subsample, colsample_bytree et min_child_samples inspirés par les papiers d'Applied Energy
        ...
        # Entraînement avec spécification explicite des variables catégorielles
        model.fit(X_train_h, y_train_h, categorical_feature=categorical_cols)
        models[h] = model
    ...
    for idx in range(0, len(df_backtest) - 48, 24):
        ...
        for h in range(1, 49):
            ...
            # Forcer le type catégorie pour les colonnes qualitatives
            for col in categorical_cols:
    ...
    for h in range(1, 49):
        ...
        for col in categorical_cols:
            X_last[col] = X_last[col].astype('category')
    ...
    if val_mape is not None:
        logging.info(f"==> SCORE MAPE DE VALIDATION (LightGBM) : {val_mape:.4f} %")
        ...
        with open(log_path, 'a', encoding='utf-8') as f:
            if not log_exists:
                f.write("# Historique des Expérimentations XGBoost / LightGBM\n\n")
                f.write("| Date | Modèle | MAPE (%) | Description des Features | Estimators | num_leaves | LR |\n")
                f.write("|---|---|---|---|---|---|---|\n")
            
            features_desc = "LightGBM with native categorical handling (Weather + Calendar FR)"
            f.write(f"| {datetime.now().strftime('%Y-%m-%d %H:%M')} | **LightGBM** | **{val_mape:.3f} %** | {features_desc} | 110 | 31 | 0.06 |\n")
    ...
    logging.info(f"Insertion de {len(unique_insert_rows)} prédictions LightGBM en base...")
    execute_values(cursor, """
        INSERT INTO predictions_lightgbm (timestamp, predicted_value, model_name, horizon, prediction_date)
        VALUES %s
    ...
    conn.commit()
    logging.info("Prévisions LightGBM insérées avec succès dans Supabase !")
    
    cursor.close()
    conn.close()
    logging.info("--- Fin du pipeline LightGBM ---")
